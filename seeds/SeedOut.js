...
const operationsDoc = `
  mutation MyMutation($match_id: uuid, $player_id: uuid, $is_out: Boolean) {
    update_Players_Batting_stats(where: {Match: {id: {_eq: $match_id}}, Player: {id: {_eq: $player_id}}}, _set: {is_out: $is_out}) {
      affected_rows
      returning {
        is_out
      }
...
function executeMyMutation(match_id, player_id, is_out) {
  return fetchGraphQL(
    operationsDoc,
    "MyMutation",
    {"match_id": match_id, "player_id": player_id, "is_out": is_out}
  );
}

async function startExecuteMyMutation(match_id, player_id, is_out) {
  const { errors, data } = await executeMyMutation(match_id, player_id, is_out);
...
async function seedData() {
    ...
    file_names.forEach(async (file) => {
        ...
        playerNames.forEach(async (name) => {
            ...
            // console.log(match_id, player_id, is_out)
            // break;
            await startExecuteMyMutation(match_id, player_id, is_out);
